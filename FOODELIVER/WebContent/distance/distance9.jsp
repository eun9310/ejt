...
<script src="https://api2.sktelecom.com/tmap/js?version=1&format=javascript&appKey=l7xx93b7c527952f4d53a46c0c1fba3a37ce"></script>
        ...
        function initTmap(){
        	...
        	if (navigator.geolocation) {
           	...
           	var startY = lat;
            var startX = lon;
            var endY = <%=orderarraylist.get(8).getCompany_address_lat()%>;
            var endX = <%=orderarraylist.get(8).getCompany_address_lng()%>;
            var passList = null;
            var prtcl;
            var headers = {};
            ...
            // 구글 지도에서 나오는 좌표의 x, y를 바꾸면 된다.
           	var startY = <%=orderarraylist.get(8).getCompany_address_lat()%>;
            var endX = <%=orderarraylist.get(8).getCompany_address_lng()%>;
            var endY = <%=orderarraylist.get(8).getMember_address_lat()%>;
            var endX = <%=orderarraylist.get(8).getMember_address_lng()%>;
            var passList = null;
            var prtcl;
            var headers = {};
