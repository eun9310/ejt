...
<script src="https://api2.sktelecom.com/tmap/js?version=1&format=javascript&appKey=l7xx93b7c527952f4d53a46c0c1fba3a37ce"></script>
        ...
        function initTmap(){
        	var totalDistance='<%=request.getParameter("totalDistance")%>';
        	if(size>1){
        	if (navigator.geolocation) {
           	...
           	var startY = lat;
            var startX = lon;
            var endY = <%=orderarraylist.get(1).getCompany_address_lat()%>;
            var endX = <%=orderarraylist.get(1).getCompany_address_lng()%>;
            var passList = null;
            var prtcl;
            var headers = {};
            ...
            // 구글 지도에서 나오는 좌표의 x, y를 바꾸면 된다.
           	var startY = <%=orderarraylist.get(1).getCompany_address_lat()%>;
            var endX = <%=orderarraylist.get(1).getCompany_address_lng()%>;
            var endY = <%=orderarraylist.get(1).getMember_address_lat()%>;
            var endX = <%=orderarraylist.get(1).getMember_address_lng()%>;
            var passList = null;
            var prtcl;
            var headers = {};
            ...
            $.ajax({
                ...
                success:function(response){ //API가 제대로 작동할 경우 실행될 코드
					...
					alert('엔터를 눌러주세요');
                    // 실시간 교통정보 추가
                    location.href='<%=request.getContextPath()%>/distance/distance3.jsp?totalDistance='+totalDistance;
                },
                error:function(request,status,error){ // API가 제대로 작동하지 않을 경우
                console.log("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
                }
